...
async function fetchText(url, options = {}) {
  ...
  await OmniBox.log("info", `[4k-av][request] ${finalUrl}`);
...
async function home(params, context) {
  try {
    ...
    await OmniBox.log("info", `[4k-av][home] list=${list.length}`);
    return {
    ...
    await OmniBox.log("error", `[4k-av][home] ${error.message}`);
...
async function category(params, context) {
  ...
  try {
    ...
    await OmniBox.log("info", `[4k-av][category] type=${typeId} page=${page} sourcePage=${sourcePage} list=${list.length}`);
    return {
      page,
      pagecount: pageCount,
    ...
    await OmniBox.log("error", `[4k-av][category] ${error.message}`);
...
async function detail(params, context) {
  try {
    ...
    await OmniBox.log("info", `[4k-av][detail] ${info.vod_name} episodes=${info.vod_play_sources?.[0]?.episodes?.length || 0}`);
    return { list: [info] };
  } catch (error) {
    await OmniBox.log("error", `[4k-av][detail] ${error.message}`);
    return { list: [] };
  }
}
...
async function search(params, context) {
  ...
  try {
    ...
    await OmniBox.log("info", `[4k-av][search] keyword=${keyword} list=${list.length}`);
    return {
      page,
      pagecount: 1,
      total: list.length,
    ...
    await OmniBox.log("error", `[4k-av][search] ${error.message}`);
...
async function play(params, context) {
  try {
    ...
    if (!pageUrl) {
      return { parse: 0, jx: 0, url: "", urls: [], flag: "4k-av", header: {}, headers: {} };
    }
    ...
    await OmniBox.log("error", `[4k-av][play] ${error.message}`);
    return { parse: 0, jx: 0, url: "", urls: [], flag: "4k-av", header: {}, headers: {} };
  }
}
